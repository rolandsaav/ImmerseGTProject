...
if (leftScriptComp && leftScriptComp.api.onTriggerStart) {
    leftScriptComp.api.onTriggerStart.add(function () {
        changeImage(-1);
    });
} else {
    print("Left button Interactable not wired properly.");
}

var rightScriptComp = script.rightButton.getComponent("Component.ScriptComponent");
if (rightScriptComp && rightScriptComp.api.onTriggerStart) {
    rightScriptComp.api.onTriggerStart.add(function () {
        changeImage(1);
    });
} else {
    print("Right button Interactable not wired properly.");
}
