...
if (leftScriptComp && leftScriptComp.api.onButtonPinched) {
    leftScriptComp.api.onButtonPinched.add(function () {
        changeImage(-1);
    });
} else {
    print("🚨 Left button Interactable not wired properly or onButtonPinched missing.");
}

var rightScriptComp = script.rightButton.getComponent("Component.ScriptComponent");
if (rightScriptComp && rightScriptComp.api.onButtonPinched) {
    rightScriptComp.api.onButtonPinched.add(function () {
        changeImage(1);
    });
} else {
    print("🚨 Right button Interactable not wired properly or onButtonPinched missing.");
}
