...
function setupButtons() {
    ...
    if (leftScript && leftScript.api) {
        if (leftScript.api.onButtonPinched && typeof leftScript.api.onButtonPinched.add === "function") {
            print("✅ Connected to onButtonPinched (left)");
            ...
            print("✅ Connected to onTriggerStart (left)");
            leftScript.api.onTriggerStart.add(function() {
                print("◀️ Left button pressed via trigger!");
                changeImage(-1);
            });
        } else {
            print("⚠️ Left button script found, but no usable interaction event");
        }
    } else {
        print("❌ Left button script not found or api missing");
    }
    ...
    if (rightScript && rightScript.api) {
        if (rightScript.api.onButtonPinched && typeof rightScript.api.onButtonPinched.add === "function") {
            print("✅ Connected to onButtonPinched (right)");
            ...
            print("✅ Connected to onTriggerStart (right)");
            rightScript.api.onTriggerStart.add(function() {
                print("▶️ Right button pressed via trigger!");
                changeImage(1);
            });
        } else {
            print("⚠️ Right button script found, but no usable interaction event");
        }
    } else {
        print("❌ Right button script not found or api missing");
    }
}
}
