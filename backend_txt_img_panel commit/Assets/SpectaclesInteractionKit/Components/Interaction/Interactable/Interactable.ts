...
export class Interactable extends BaseScriptComponent {
  ...
  // Add a simple button pinched event for easier access from JavaScript
  private onButtonPinchedEvent = new Event<void>()
  ...
  /**
   * Simplified event that fires when this interactable is triggered/pinched
   * Designed for easier access from JavaScript code
   */
  onButtonPinched = this.onButtonPinchedEvent.publicApi()
  ...
  triggerStart = (eventArgs: InteractableEventArgs): void => {
    ...
    if (this._triggeringInteractor === InteractorInputType.None) {
      ...
      // Fire the simplified button pinched event
      this.onButtonPinchedEvent.invoke()
      this.log.v("InteractionEvent : " + "On Button Pinched Event")
    }
